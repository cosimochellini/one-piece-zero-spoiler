...
fog" and never the name; the page body then follows the live dial.
...
draws its route position against `chartWith(entity)`, the chart with that
record set in at its threshold when it is not already drawn, so an unlisted
character still has a waypoint number while the reader is looking at them.
